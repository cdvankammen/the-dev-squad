...
export default function SquadPage() {
  ...
  const [queuedMessages, setQueuedMessages] = useState<Record<AgentId, Array<{ id: string; text: string }>>>({
    A: [], B: [], C: [], D: [], E: [], S: [],
  });
  ...
  async function dispatchDirectMessage(agent: AgentId, message: string) {
  ...
  async function handleSend() {
    ...
    if (sendingAgents.has(selectedAgent)) {
      setQueuedMessages((prev) => ({
        ...prev,
        [selectedAgent]: [...prev[selectedAgent], { id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, text: message }],
      }));
      setChatInput('');
      return;
    }
  ...
  async function sendQueuedMessage(agent: AgentId, itemId: string) {
    ...
    if (!queued || sendingAgents.has(agent)) return;
            ...
            <div className="shrink-0 border-t border-white/10 px-4 py-3">
              ...
              {queuedMessages[selectedAgent].length > 0 && (
                <div className="mt-3 space-y-2 rounded-xl border border-white/10 bg-white/[0.03] p-3">
                  <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Pending queue</div>
