// Generated by scripts/build-runtime-lib.mjs from src/lib/jsonRepair.ts
/**
...
function removeComments(value) {
    ...
    for (let i = 0; i < value.length; i += 1) {
        ...
        if (char === '/' && value[i + 1] === '*') {
            i += 2;
            while (i < value.length - 1 && !(value[i] === '*' && value[i + 1] === '/'))
                i += 1;
            i += 1;
            continue;
        }
...
function isToolLike(value) {
    ...
    if (node.function && isObject(node.function)) {
        return Boolean(node.function.name || node.function.arguments || node.function.input);
    }
...
function visitToolNodes(node, results) {
    ...
    if (obj.message)
        visitToolNodes(obj.message, results);
    if (Array.isArray(obj.content))
        visitToolNodes(obj.content, results);
    if (isToolLike(obj))
        results.push(obj);
}
...
export function extractStructuredToolCallsFromText(text) {
    ...
    for (const candidate of collectJsonCandidates(text)) {
        if (!/tool_calls?|tool_use|name|tool|function|arguments|input|params/i.test(candidate))
            continue;
        try {
