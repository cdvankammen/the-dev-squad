...
const HTTP_REQUEST_RETRIES = parseInt(process.env.HTTP_SHIM_REQUEST_RETRIES || '3', 10);
const HTTP_RETRY_BASE_DELAY_MS = parseInt(process.env.HTTP_SHIM_RETRY_BASE_DELAY_MS || '1500', 10);
const HTTP_RETRY_MAX_DELAY_MS = parseInt(process.env.HTTP_SHIM_RETRY_MAX_DELAY_MS || '10000', 10);
...
function computeRetryDelayMs(attempt) {
  const backoff = Math.min(HTTP_RETRY_MAX_DELAY_MS, HTTP_RETRY_BASE_DELAY_MS * (2 ** Math.max(0, attempt - 1)));
...
function isRetryableHttpFailure(status, bodyText = '') {
  if (RETRYABLE_HTTP_STATUSES.has(Number(status))) return true;
  const body = String(bodyText || '').toLowerCase();
  return (
    body.includes('model is loading') ||
    body.includes('loading model') ||
    body.includes('model loading') ||
...
async function requestChatCompletionsWithRetry(baseUrl, headers, requestBody, requestLabel = 'chat/completions') {
  let attempt = 0;
  ...
  while (true) {
    try {
      ...
      if (!response.ok && attempt < HTTP_REQUEST_RETRIES && isRetryableHttpFailure(response.status, raw)) {
        attempt += 1;
        const delay = computeRetryDelayMs(attempt);
        debug(`[retry] ${requestLabel} HTTP ${response.status}; attempt ${attempt}/${HTTP_REQUEST_RETRIES}; waiting ${delay}ms`);
        await sleep(delay);
        continue;
      }
      ...
      if (attempt < HTTP_REQUEST_RETRIES && isRetryableNetworkError(err)) {
        attempt += 1;
        const delay = computeRetryDelayMs(attempt);
        debug(`[retry] ${requestLabel} network error; attempt ${attempt}/${HTTP_REQUEST_RETRIES}; waiting ${delay}ms; error=${err?.message || String(err)}`);
        await sleep(delay);
        continue;
      }
      throw err;
...
function shouldSkipGrepDirectory(name) {
  return new Set(['.git', '.next', 'node_modules', '.turbo', 'dist', 'build', 'coverage']).has(String(name || ''));
}
...
function validateAgainstSchema(value, schema, currentPath = '$') {
  ...
  if (expectedType && !schemaTypeMatches(expectedType, value)) {
    const expectedLabel = Array.isArray(expectedType) ? expectedType.join(' | ') : String(expectedType);
    const actualLabel = Array.isArray(value) ? 'array' : value === null ? 'null' : typeof value;
    return `${currentPath} must be ${expectedLabel} (received ${actualLabel})`;
  }
  ...
  if (schema.type === 'object' && isPlainObject(value)) {
    ...
    for (const key of required) {
      if (!(key in value)) {
        return `${currentPath}.${key} is required`;
      }
    }
    ...
    for (const [key, propertySchema] of Object.entries(properties)) {
      ...
      const childError = validateAgainstSchema(value[key], propertySchema, `${currentPath}.${key}`);
  ...
  if (schema.type === 'array' && Array.isArray(value) && schema.items) {
    for (let index = 0; index < value.length; index += 1) {
      const childError = validateAgainstSchema(value[index], schema.items, `${currentPath}[${index}]`);
      if (childError) return childError;
    }
  }
...
function validateToolArguments(toolName, value) {
  const entry = getToolRegistryEntry(toolName);
  if (!entry?.parameters) return null;
  return validateAgainstSchema(value, entry.parameters, '$');
}

function buildToolRepairPrompt(toolName, schemaText, originalArgs, problemText) {
  return `The last ${toolName} tool call could not be executed because its arguments were invalid.\n\nProblem: ${problemText}\nOriginal arguments: ${String(originalArgs ?? '').slice(0, 1200)}\nSchema: ${schemaText}\n\nPlease fix this by calling the ${toolName} tool again with corrected arguments. If your model cannot emit tool calls, respond with ONLY a single JSON object containing valid ${toolName} arguments. Do not include markdown fences or extra commentary.`;
}
...
async function repairToolArgumentsWithModel({
  ...
  const { response, raw } = await requestChatCompletionsWithRetry(baseUrl, headers, {
      model: repairModel,
      stream: false,
      temperature: 0,
      messages: [
        {
          role: 'system',
          content: 'You repair malformed tool-call arguments. Return only a JSON object with valid tool arguments. No markdown. No commentary.',
        },
        {
          role: 'user',
  ...
  if (!response.ok) {
    throw new Error(`Repair model request failed (${response.status}): ${raw || response.statusText}`);
  }

  const payload = JSON.parse(raw);
  const content = payload?.choices?.[0]?.message?.content || '';
  const repaired = parsePossiblyMalformedJson(typeof content === 'string' ? content : JSON.stringify(content));
  const validationError = validateToolArguments(toolName, repaired);
  if (validationError) {
    throw new Error(`Repair model returned invalid arguments: ${validationError}`);
  }
  return repaired;
}
        ...
        try {
          ...
          function searchFile(filePath) {
            try {
              ...
              for (let i = 0; i < lines.length; i++) {
                ...
                if (match) {
                  results.push(`${path.relative(process.cwd(), filePath)}:${i + 1}: ${line.trim()}`);
          ...
          function walkGrep(targetPath) {
            ...
            for (const e of entries) {
              ...
              if (e.isDirectory()) {
                if (shouldSkipGrepDirectory(e.name)) continue;
                walkGrep(p);
...
When your task is complete, provide a clear summary of what you did.`;
  ...
  let finalResult = '';
  let turn = 0;
  const toolArgRepairModel = process.env.HTTP_SHIM_TOOL_ARG_REPAIR_MODEL || process.env.HTTP_SHIM_ARG_REPAIR_MODEL || '';
  let pendingToolRepair = null;
  ...
  while (turn < MAX_AGENT_TURNS) {
    ...
    try {
      ...
      const { response, raw } = await requestChatCompletionsWithRetry(
        baseUrl,
        headers,
        requestBody,
        supportsTools ? 'agent-turn-with-tools' : 'agent-turn',
      );
      if (process.env.HTTP_SHIM_DEBUG === '1') {
      ...
      if (!response.ok) {
        ...
        if (supportsTools && (response.status === 400 || response.status === 422)) {
          ...
          const { response: retryResponse, raw: retryRaw } = await requestChatCompletionsWithRetry(
            baseUrl,
            headers,
            requestBody,
            'agent-turn-no-tools-fallback',
          );
          if (!retryResponse.ok) {
    ...
    if ((!toolCalls || toolCalls.length === 0) && textContent && supportsTools) {
      if (pendingToolRepair?.toolName) {
        try {
          const repairedArgs = parsePossiblyMalformedJson(textContent);
          const validationError = validateToolArguments(pendingToolRepair.toolName, repairedArgs);
          if (!validationError) {
            effectiveToolCalls = normalizeToolCalls([
              {
                id: `tool-repair-${crypto.randomUUID()}`,
                function: {
                  name: pendingToolRepair.toolName,
                  arguments: JSON.stringify(repairedArgs),
                },
              },
            ]).filter((tc) => isKnownToolName(tc?.function?.name));
            debug(`Recovered ${pendingToolRepair.toolName} tool call from repair JSON response`);
          }
        } catch {
          // fall through to normal text-based tool-call extraction
        }
      }
    ...
    for (const tc of effectiveToolCalls) {
      ...
      const registryEntry = getToolRegistryEntry(toolName);
      const schemaText = JSON.stringify(registryEntry?.parameters || {});
      const originalArgs = tc.function?.arguments || '{}';

      // Parse/repair tool arguments before executing
      let toolInput;
      try {
        toolInput = parsePossiblyMalformedJson(originalArgs);
      } catch (err) {
        const problemText = `JSON parse failed: ${err.message || String(err)}`;
        const msg = `Invalid tool arguments for ${toolName}: ${problemText}\nOriginal: ${String(originalArgs).slice(0, 800)}`;
        debug(msg);
        ...
        if (!pendingToolRepair) {
          pendingToolRepair = { toolName };
          repairPrompt = buildToolRepairPrompt(toolName, schemaText, originalArgs, problemText);
          needRetry = true;
          // Stop processing further tool calls in this turn so we can let the model fix the arguments
          break outerToolLoop;
        }

        if (toolArgRepairModel) {
          try {
            toolInput = await repairToolArgumentsWithModel({
              baseUrl,
              headers,
              repairModel: toolArgRepairModel,
              toolName,
              schemaText,
              originalArgs,
              problemText,
            ...
            const repairMsg = `Repair model failed for ${toolName}: ${repairError.message || String(repairError)}`;
            debug(repairMsg);
            emit({
              type: 'user',
              message: {
            ...
            toolResultMessages.push({ role: 'tool', tool_call_id: toolUseId, content: repairMsg });
            continue;
          }
        } else {
          continue;
      ...
      const validationError = validateToolArguments(toolName, toolInput);
      if (validationError) {
        const problemText = `Schema validation failed: ${validationError}`;
        const msg = `Invalid tool arguments for ${toolName}: ${problemText}\nOriginal: ${String(originalArgs).slice(0, 800)}`;
        debug(msg);
    ...
    // We handled tool calls this turn (including text-extracted fallback tool calls),
    // so continue the loop and give the model a chance to consume tool results on
    // the next round even if the provider reported finish_reason='stop'.
  }
