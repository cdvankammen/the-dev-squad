 ...
 * identifier. Small models benefit from simplified role prompts.
 */
function isSmallModel(model: string): boolean {
  const m = String(model || '').toLowerCase();

  if (!m) return false;
  // Anthropic model aliases are not treated as "small" here.
  ...
  if (sizeMatch) {
    ...
    if (Number.isFinite(sizeB) && sizeB <= 4) return true;
  }
